...
common_prog(int nargs, char **args)
{
	...
	/* TODO: waitpid here instead of sleeping */
	//clocksleep(6);
	int ret;
	int* stat;
	int res = waitpid(new->processid, stat, 0, &ret);
...
/*
 * Command for printing the DBFLAGS.
 */
...
int firstByte, secondByte, thirdByte;
	firstByte = dfSwitch[0]*1 + dfSwitch[1]*2 + dfSwitch[2]*4 + dfSwitch[3]*8;
	secondByte = dfSwitch[4]*1 + dfSwitch[5]*2 + dfSwitch[6]*4 + dfSwitch[7]*8;
	thirdByte = dfSwitch[8]*1 + dfSwitch[9]*2 + dfSwitch[10]*4 + dfSwitch[11]*8;
	kprintf("Current value of dbflags is 0x");

	if(thirdByte != 0){
		kprintf("%x", thirdByte);
	}
	if(secondByte!=0)
		kprintf("%x", secondByte);
	kprintf("%x\n", firstByte);
	return 0;
return 0;
}
...
{
	if(atoi(args[1]) < 1 || atoi(args[1]) > 12 || nargs != 3){
		kprintf("Usage: df nr on/off\n");
		return 0;	   	
	}
	
	if(strcmp(args[2], "on") == 0){
		dfSwitch[atoi(args[1])-1] = 1;	
	}
	else if(strcmp(args[2], "off") == 0){
		dfSwitch[atoi(args[1])-1] = 0;
	}
	else{
		kprintf("Usage: df nr on/off\n");
	}
