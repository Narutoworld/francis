...
#define KVADDR_TO_PAGE_FRAME(kvaddr) (((kvaddr)-MIPS_KSEG0)/PAGE_SIZE - first_coremap_page_num)
#define KVADDR_TO_PADDR(kvaddr) ((kvaddr)-MIPS_KSEG0)

#define NUM_PAGE 1024*1024
struct process {
    //the parent's process id of this process
    pid_t parentPid;
    //points to the thread belong to this process
    struct thread *mythread;

    struct semaphore* exit_wait_lock;
...
////////////////////////function declarations for process/////////////////////////////////////////
...
//    unsigned int wtf1;
    ...
    unsigned int copy;
    int first_level_entry;
    int second_level_entry;
};

struct coremap* coremapTable;

void coremap_table_bootstrap();

struct page_table_entry{
    /*kernel address*/
	unsigned page_frame_address:32;
};
struct addrspace;
